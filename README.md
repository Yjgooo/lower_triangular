...
The script will:
- simulate data (configurable at the top of `main()`),
...
## Visualizations
Plotting is off by default to keep core logic clean.
- In `main()`, set the visualization toggle to `True` to see:
  - `plot_true_curves`: true `a(t)` and `b(j)` lines.
  - `plot_true_mean_surface`: 2D heatmap of the triangular mean surface.
  - `plot_true_mean_surface_3d`: 3D surface plot (flip its inner toggle to `True`).
