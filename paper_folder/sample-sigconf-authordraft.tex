...
A product change can affect users differently across repeated exposures. An intervention may attenuate as initial novelty fades, or it may strengthen as users become familiar with a new interface. In such settings, a single average treatment-effect summary can be difficult to interpret and may be inadequate for product decisions. Standard experimentation practice either treats the treatment effect as effectively exposure-invariant or collapses over exposure history into a single summary, and therefore cannot answer rollout questions when user response adapts over repeated exposure.
...
Our theoretical analysis draws on standard asymptotic arguments for finite-dimensional estimators and on the classical bootstrap. The bootstrap originates with \cite{Efron1979}; general large-sample arguments for consistency, asymptotic normality, and the delta method may be found in \cite{vdV1998}. Because our data are clustered at the user level, resampling must preserve the within-user dependence structure; the user-level bootstrap is consistent in this setting under standard regularity conditions, and our argument is closely related to the general clustered-data discussion in \cite{FieldWelsh2007}.
